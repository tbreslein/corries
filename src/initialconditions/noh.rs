...
pub fn init_noh<const S: usize, const EQ: usize>(u: &mut Physics<S, EQ>) -> Result<()>{
    let supported_physics = [PhysicsMode::Euler1DAdiabatic, PhysicsMode::Euler1DIsot, PhysicsMode::Euler2DIsot];
    if !supported_physics.contains(&u.mode) {
        bail!("Noh initial conditions do not support your chosen PhysicsMode! Got mode = {:?}", u.mode);
    }
